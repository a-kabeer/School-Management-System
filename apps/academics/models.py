...
class TeacherAssignment(BranchOwnedModel):
    ...
    def clean(self):
        super().clean()
        if self.section_id and self.class_subject_id and self.section.school_class_id != self.class_subject.school_class_id: raise ValidationError({"section": _("That section belongs to a different class.")})
...
class Timetable(BranchOwnedModel):
    ...
    class Meta:
        verbose_name = _("timetable slot"); verbose_name_plural = _("timetable"); ordering = ["weekday", "period"]; constraints = [models.UniqueConstraint(fields=["section", "weekday", "period", "academic_year"], name="uq_timetable_slot"), models.CheckConstraint(condition=models.Q(end_time__gt=models.F("start_time")), name="ck_timetable_times")]; indexes = [models.Index(fields=["branch", "academic_year", "weekday"]), models.Index(fields=["teacher", "weekday"])]
    def __str__(self): return f"{self.section} — {self.get_weekday_display()} P{self.period}"
    def clean(self):
        super().clean()
        if self.weekday is not None and not is_working_day(self.weekday, self.branch): raise ValidationError({"weekday": _("This day is not configured as a working day for the branch.")})
    def save(self, *args, **kwargs):
        if self.weekday is not None and not is_working_day(self.weekday, self.branch): raise ValidationError(_("This day is not configured as a working day for the branch."))
