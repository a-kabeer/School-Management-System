...
class AcademicsCreateView(PermissionRequiredMixin, View):
    ...
    def get(self, request, *args, **kwargs):
        ...
        if not key:
            return TemplateResponse(request, self.template_name, {"resources": RESOURCES})
        model = RESOURCES[key][1]
        if not user_has_permission(request.user, f"academics.add_{model._meta.model_name}", getattr(request, "active_branch", None)):
